...
import { createBorrowing, approveBorrowing, returnBorrowing, extendBorrowing, reserveBook, listBorrowings, getBorrowing, quickBorrow, quickReturn, searchStudent } from '../../controllers/borrowingController';
...
const reserveSchema = { body: Joi.object({ book_qr_id: Joi.number().integer().positive().required() }) };
...
router.post('/reserve', authenticate, validate(reserveSchema), reserveBook);
